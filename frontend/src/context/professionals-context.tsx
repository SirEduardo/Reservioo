...
const ProfessionalsContext = createContext<ProfessionalContextType | undefined>(undefined)

export const useProfessionals = () => {
    const context = useContext(ProfessionalsContext)
    if (!context) throw new Error('useProfessionals debe usarse dentro de un ProfessionalsProvider')
    return context
  }
...
export const ProfessionalsProvider = ({children}: {children: ReactNode}) => {
    ...
    const fetchProfessionals = async () => {
        ...
        try {
            if (!companyId) return
            const response = await fetch(`http://localhost:3100/api/professionals/${companyId}`, {
              method: "GET"
            })            
      ...
      const handleDeleteProfessional = async (professionalId: string) => {
        try {
          const response = await fetch(`http://localhost:3100/api/professional/${professionalId}`, {
            method: "DELETE"
          });
          ...
          setProfessionals(prevProfessionals =>
            prevProfessionals.filter(professional => professional.id !== professionalId)
          );
        } catch (error) {
...
return (
    <ProfessionalsContext.Provider value={{loading, error, professionals, handleAddProfessional, fetchProfessionals, handleDeleteProfessional}}>
        {children}
